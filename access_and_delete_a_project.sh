...
{
        result=$(curl --insecure -X DELETE --header "Content-Type:application/json" --header "Authorization: bearer $bearer_token" "$web_url/api/projects/$project_id")
        if [ "$result" -eq "204" ]
        then
                # Fail 
                echo "FAILED: Could not delete project. Check user permissions"
        else
                # Success
                echo "Project with project id: $project_id deleted successfully"
        fi
}
